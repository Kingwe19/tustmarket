    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <el-button
            size="mini"
            @click="handleEdit(scope.$index, scope.row)">修改介绍
          </el-button>
          <el-button
            size="mini"
            @click="handleEdit2(scope.$index, scope.row)">取消出售
          </el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog
      ...
      <div style="text-align: center">
        <el-form :model="goodsForm">
          ...
          <el-form-item label="商品价格" label-position="top">
...
<script>
  export default {
    ...
    mounted() {
      ...
      let self = this;
      //加载文字数据
      $.get("http://localhost:8083/goods/getMyGoods", jsonObj, function (data) {
        self.tableData = data;
        //为每个表格元素加载图片数据，主图
    ...
    methods: {
      ...
      handleEdit(index, row) {
        ...
        $.get("http://localhost:8083/goods/getGoodsById", jsonObj, function (data) {
          if (data !== "" || data !== null) {
            ...
            self.goodsForm.goodsName = data.goodsName;
            self.goodsForm.goodsDescription = data.goodsDescription;
            self.goodsForm.goodsPrice = data.goodsPrice;
      ...
      handleEdit2(index, row) {
        ...
        let jsonMsg = JSON.stringify(jsonObj);
        let self = this;
        $.post("http://localhost:8083/goods/deleteMyGoodsAndUrls", jsonMsg, function (data) {
          if (data.code === 1) {
      ...
      submitDscrip() {
        ...
        jsonObj.goodsName = this.goodsForm.goodsName;
        jsonObj.goodsPrice = this.goodsForm.goodsPrice;
        jsonObj.goodsDescription = this.goodsForm.goodsDescription;
        let jsonMsg = JSON.stringify(jsonObj);
        let self = this;
        $.post("http://localhost:8083/goods/saveGoodsDscripAndGoodsName", jsonMsg, function (data) {
          if (data.code === 1) {
