  ...
  <div class="index-body">
    <el-row :gutter="20" style="margin-top: 30px;">
      <el-col :span="24">
        ...
        <div v-show="changeDisplay">
          <el-row style="margin-top: 30px;">
            <el-carousel direction="horizontal" :autoplay="true" height="370px" type="card">
        ...
        <p class="index-body-p-1">{{ dataText1 }}</p>
        <hr/>

        <el-row style="margin-top: 50px;" v-for="(obj1, index1) in getRowNums(data1)" :key="getRandomString(20)">
          <h2 v-show="data1.length === 0" style="width: 100%;text-align: center">该分类下，没有商品 ~</h2>
        ...
        <div v-show="changeDisplay">
          ...
          <el-row style="margin-top: 50px;" v-for="(obj1, index1) in getRowNums(data2)" :key="getRandomString(20)">
            <el-col :span="4" v-for="(obj2, index2) in mySlice(data2,index1*4,(index1+1)*4)" :key="getRandomString(21)"
                    :offset="index2 > 0 ? 2 : 1">
              <el-card :body-style="{ padding: '0px', textAlign: 'center'}">
                <img :src="obj2.picture" class="image" style="width: 100%" alt="请检查网络连接">
                <div style="padding: 10px;">
    ...
    <el-dialog
      ...
      <div style="text-align: center">
        ...
        <span slot="footer" class="dialog-footer">
          <el-button type="primary" @click="addToShopCar" style="margin-top: 30px;">加入收藏</el-button>
        </span>
      </div>
    </el-dialog>
    <!--      加入收藏结果提示框-->
        ...
        <span slot="footer" class="dialog-footer">
          <el-row style="margin-top: 30px;">
            ...
            <el-col :span="6" :offset="2">
              <el-button type="primary" @click="clickGoToShopCar" :disabled="goToShopCar">前往我的收藏</el-button>
            </el-col>
          </el-row>
        </span>
      </div>
  ...
  export default {
   ...
   /* 加载热门精品，九新以上，低价好物*/
    mounted() {
      ...
      //加载热门精品数据，服务端返回最多 12条 数据
      $.get("http://localhost:8083/goods/getIndexGoods/1", function (data) {
        if(self.$route.params.selectNum !== undefined){
          let jsonObj = {};
          jsonObj.selectNum = self.$route.params.selectNum;
          $.get("http://localhost:8083/goods/getSelectGoods", jsonObj, function (data) {
            //保存搜索内容
          ...
          switch (self.$route.params.selectNum) {
            case '1':self.dataText1 = '服饰';break;
            case '2':self.dataText1 = '配饰';break;
            case '3':self.dataText1 = '鞋子';break;
            case '4':self.dataText1 = '美妆洗护';break;
            case '5':self.dataText1 = '电子数码';break;
            case '6':self.dataText1 = '运动户外';break;
            case '7':self.dataText1 = '图书文娱';break;
            case '8':self.dataText1 = '演出票';break;
            case '9':self.dataText1 = '生活旅行';break;
            case '10':self.dataText1 = '箱包烟酒';break;
            case '11':self.dataText1 = '宠物';break;
            case '12':self.dataText1 = '其他';break;
          }
      ...
      $.get("http://localhost:8083/goods/getIndexGoods/2", function (data) {
        self.data2 = data;
      ...
      $.get("http://localhost:8083/goods/getIndexGoods/3", function (data) {
        self.data3 = data;
    ...
    methods: {
      ...
      clickGoodsInfoButton(goodsId) {
        ...
        $.get("http://localhost:8083/goods/getGoodsById", jsonObj, function (data) {
          ...
          self.goodsInfoDscrip = data.goodsDescription;
          $.get("http://localhost:8083/goods/getGoodsImgMap", jsonObj, function (data) {
      ...
      addToShopCar() {
        ...
        let jsonMsg = JSON.stringify(jsonObj);
        let self = this;
        $.post("http://localhost:8083/collection/addOneToCollection", jsonMsg, function (data) {
          if (data.code === 1) {
            self.dialogValue = "加入我的收藏成功";
            self.goToShopCar = false;
          } else {
            self.dialogValue = "加入我的收藏失败，错误码：" + data.code + ";商品已经存在";
      ...
      clickSearch() {
        if (this.search !== "") {
          ...
          let jsonObj = {};
          jsonObj.goodsName = this.search;
          $.get("http://localhost:8083/goods/getSearchGoods", jsonObj, function (data) {
            //保存搜索内容
            self.dataSearch = data;
            $(self.dataSearch).each(function (index, element) {
              let jsonObj = {};
