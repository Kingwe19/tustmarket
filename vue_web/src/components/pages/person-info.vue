        ...
        <el-row :gutter="100">
          <el-col :span="9" :offset="2">
            <el-form-item label="用户名">
              <el-input v-model="formLabelAlign.usernameTel" disabled></el-input>
            </el-form-item>
          </el-col>
        ...
        <el-row :gutter="100">
          <el-col :span="9" :offset="2">
            <el-form-item label="邮箱">
              <el-input v-model="formLabelAlign.usernameEmail"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="9" :offset="1">
            <el-form-item label="账户验证状态">
              <el-input v-model="formLabelAlign.openClose" disabled></el-input>
            </el-form-item>
          </el-col>
        </el-row>
...
<script>
  export default {
    ...
    mounted() {
      ...
      let self = this;
      $.get("http://localhost:8083/user/getInfo", jsonObj, function (data) {
        self.formLabelAlign.usernameTel = data.usernameTel;
        if (data.sex === 1)
        ...
        self.formLabelAlign.openClose = "已启用";
        self.formLabelAlign.usernameEmail = data.usernameEmail;
      }, "json");
    },
    methods: {
      // submitForm(){
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            // alert('submit!');
            if(this.formLabelAlign.sex === "女")
              this.formLabelAlign.sex = 2;
            else
              this.formLabelAlign.sex = 1;
            this.formLabelAlign.userId = window.sessionStorage.getItem("userId");
            this.formLabelAlign.openClose = 1;
            let jsonMsg = JSON.stringify(this.formLabelAlign);
            let self = this;
            $.post("http://localhost:8083/user/setInfo", jsonMsg, function (data) {
              if (data.code === 1) {
